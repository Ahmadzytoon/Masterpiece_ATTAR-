...
<div class="row container m-auto">
    ...
    <div class="col-12">
        <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Add discount for product</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form action="{{route('admin.product_discount.store')}}" method="POST" enctype="multipart/form-data">
              @csrf

              <div class="card-body">
                <div class="form-group">
                    <label for="exampleSelectRounded0">Products name<code></code></label>
                    <select class="custom-select rounded-0" id="exampleSelectRounded0" name="select_product"   class="@error('select_product') is-invalid @enderror">
                        @foreach($products as $value)
                            <option value="{{$value->id}}">{{$value->namepro}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label for="exampleInputEmail1">Discount mount %</label>
                    <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Enter  discount" name="discount_product"  class="@error('discount_product') is-invalid @enderror">
